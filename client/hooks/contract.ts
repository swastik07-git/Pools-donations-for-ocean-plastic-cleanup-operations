...
/**
 * Initialize a voting poll with candidates.
 * Calls: init(candidates: Vec<String>)
 */
export async function initializePoll(
  caller: string,
  candidates: string[]
) {
  // Build ScVal Vec<String> - use scvVec static method
  const vec = xdr.ScVal.scvVec(candidates.map(c => toScValString(c)));
  return callContract(
    "init",
    [vec],
    caller,
...
/**
 * Get vote count for a candidate (read-only).
...
/**
 * Get current winner (read-only).
 * Calls: get_winner() -> String
 */
export async function getWinner(caller?: string) {
  return readContract(
    "get_winner",
    [],
    caller
